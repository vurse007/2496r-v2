...
struct PursuitParams {
    ...
    double base_lookahead = 12.0;        // Base lookahead distance (inches)
    double min_lookahead = 6.0;          // Minimum lookahead (sharp turns)
    double max_lookahead = 20.0;         // Maximum lookahead (straight paths)
    double curvature_scale = 0.8;        // How much curvature affects lookahead
    
    // Heading control parameters
    double heading_kp = 1.5;             // Heading correction gain
    double heading_blend_dist = 10.0;    // Distance to start blending heading
    double heading_blend_power = 2.0;    // Exponential blend curve (higher = later blend)
    
    // Path following parameters
    double path_completion_dist = 2.0;   // Distance to consider path complete (inches)
    double final_heading_tolerance = 3.0; // Final heading tolerance (degrees)
